...
The function should return the first object with the key of username that matches the 
...
function removeUser(usersArray, username) {
  let user = usersArray.findIndex((a)=>{
    return a["username"] === username;
  });
  if(user === -1) return;
  let deleted = usersArray[user];
